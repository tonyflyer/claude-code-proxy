    ...
    def validate_model_field(cls, v, info):  # Renamed to avoid conflict
        ...
        # Add prefixes to non-mapped models if they match known lists
            ...
            if clean_v in GEMINI_MODELS and not v.startswith("gemini/"):
                ...
                mapped = True  # Technically mapped to add prefix
            elif PREFERRED_PROVIDER == "openrouter" and not v.startswith("openrouter/"):
...
async def create_message(request: MessagesRequest, raw_request: Request):
    try:
        ...
        if request.model.startswith("openrouter/"):
            ...
            if OR_APP_NAME:
                if "extra_body" not in litellm_request:
                    litellm_request["extra_body"] = {}
                litellm_request["extra_body"]["X-Title"] = OR_APP_NAME
            logger.debug(
                f"Using OpenRouter API key and base URL {OPENROUTER_API_BASE} for model: {request.model}"
            )
